...
- **이번에는 클라이언트 응답 유실과 이후 안전한 재시도를 검증했다.** §3의 시나리오 9가 "커밋은 됐는데 클라이언트가 응답을 못 받는" 상황을 실제로 재현하고(curl 타임아웃을 직접 확인), 그 뒤 재시도가 추가 쓰기 없이 같은 결과를 반환함을 확인했다. `coupon_award.status`가 이미 REDEEMED면 추가 쓰기 없이 기존 결과를 그대로 반환하므로, 클라이언트는 응답을 못 받았을 때 그냥 같은 요청을 다시 보내면 된다.
